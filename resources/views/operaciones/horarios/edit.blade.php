...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <form id="formulario" method="POST" action="{{ route('horarios.update', ['idHorario' => $horario->id, 'idProfesor' => $profesor->id]) }}">
            <div class="card-body">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" name="lugar" id="lugar" placeholder="Lugar"
                            value="{{ request()->old('lugar', $horario->lugar) }}">
                        @error('lugar')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="materia_id" id="materia" class="form-select">
                            ...
                            @foreach ($materias as $materia)
                                <option {{ $materia->id == old('materia_id', $horario->materia_id) ? 'selected' : '' }}
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select select-dia" name="dia_semana">
                            ...
                            @foreach ($diasSemana as $diaSemana)
                                <option
                                {{ $diaSemana['value'] === $horario->dia_semana->value ? 'selected' : ''}}
                                {{ "data-dia-value='{$diaSemana['value']}'" }}
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input class="form-control hora-inicio" name="hora_inicio" value="{{ old('hora_inicio', $horario->hora_inicio) }}"/>
                        @error('hora-incio')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <input class="form-control hora-fin" name="hora_fin" value="{{ old('hora_fin', $horario->hora_fin) }}"/>
                        @error('hora_fin')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
